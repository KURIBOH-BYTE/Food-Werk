...
from foodwerk.domain.models import (
    Category, Extra, Ingredient, MenuItem, MenuItemIngredient, User,
)
from foodwerk.data_access.dao import (
    CategoryDAO, ExtraDAO, MenuItemDAO, OrderDAO, ReviewDAO, SpecialDAO, UserDAO, AddressDAO,
)
...
def daos(engine):
    ...
    return {
        ...
        "extra": ExtraDAO(engine),
        "order": OrderDAO(engine),
        "address": AddressDAO(engine),
        "special": SpecialDAO(engine),
        "review": ReviewDAO(engine),
    }
...
def seeded_engine(engine):
    ...
    with Session(engine) as session:
        ...
        extra_cheese = Extra(name="Extra Cheese", price=1.50, category_id=burgers.id)
        session.add(extra_cheese)
        session.commit()
